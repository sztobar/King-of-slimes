...
public class PlayerWallSlideAbility : MonoBehaviour, IPlayerAbility {
  ...
  [SerializeField] private bool wallSlideEnabled;

  [SerializeField]
  [Tooltip("Time to leap after player is no longer clicking arrow key")]
  private float noInputUnstickTime = 0.2f;

  [SerializeField]
  [Tooltip("Time to leap after player is moving in opposite direction")]
  private float oppositeInputUnstickTime = 0.1f;

  [SerializeField] private WallSlideRaycaster wallSlideRaycaster;
  [SerializeField] private PlayerWallSlideJump jump;
